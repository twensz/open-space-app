...
describe('talksReducer function', () => {
  ...
  it('should return the talks when given by RECEIVE_TALKS action', () => {
    // arrange : mengumpulkan kebutuhan yang diperlukan untuk menguji sebuah unit
    const initialState = [];
    const action = {
      type: 'RECEIVE_TALKS',
      payload: {
    ...
    // action : aksi dari unit yang ingin diuji
    const nextState = talksReducer(initialState, action);

    // assert : memeriksa efek atau hasil dari action yang telah dilakukan oleh unit yang diuji
